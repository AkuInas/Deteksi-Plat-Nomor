...
def license_complies_format(text):
    ...
    if len(text) >= 5:
       return (text[0] in string.ascii_uppercase or text[0] in dict_int_to_char.keys()) and \
...
def format_license(text):
    ...
    """
    license_plate_ = ''
    mapping = {0: dict_int_to_char, 1: dict_int_to_char, 6: dict_int_to_char, 7: dict_int_to_char, 8: dict_int_to_char,
...
def read_license_plate(contour_img):
    """
    Read the license plate text from the given cropped image.

    Args:
        contour_img (PIL.Image.Image): Cropped image containing the license plate.

    Returns:
        tuple: Tuple containing the formatted license plate text and its confidence score.
    """

    detections = reader.readtext(contour_img)
